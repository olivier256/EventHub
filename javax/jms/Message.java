...
/**
 * The {@code Message} interface is the root interface of all JMS messages. It
 * defines the message header and the {@code acknowledge} method used for all
 * messages.
 *
 * <P>
 * Most message-oriented middleware (MOM) products treat messages as lightweight
 * entities that consist of a header and a body. The header contains fields used
 * for message routing and identification; the body contains the application
 * data being sent.
 *
 * <P>
 ...
 * <LI>Provide an API suitable for creating messages that match the format used
 * by provider-native messaging applications
 * <LI>Support the development of heterogeneous applications that span operating
 ...
 * <UL>
 * <LI>Header - All messages support the same set of header fields. Header
 ...
 * <LI>Body - The JMS API defines several types of message body, which cover the
 ...
 * <UL>
 * <LI>Stream - A {@code StreamMessage} object's message body contains a stream
 * of primitive values in the Java programming language ("Java primitives"). It
 * is filled and read sequentially.
 * <LI>Map - A {@code MapMessage} object's message body contains a set of
 * name-value pairs, where names are {@code String} objects, and values are Java
 * primitives. The entries can be accessed sequentially or randomly by name. The
 * order of the entries is undefined.
 * <LI>Text - A {@code TextMessage} object's message body contains a
 * {@code java.lang.String} object. This message type can be used to transport
 * plain-text messages, and XML messages.
 * <LI>Object - An {@code ObjectMessage} object's message body contains a
 * {@code Serializable} Java object.
 * <LI>Bytes - A {@code BytesMessage} object's message body contains a stream of
 ...
 * the other body types, which are easier to use. Although the JMS API allows
 * the use of message properties with byte messages, they are typically not
 * used, since the inclusion of properties may affect the format.
 * </UL>
 *
 * <H4>Message Headers</H4>
 *
 * <P>
 * The {@code JMSCorrelationID} header field is used for linking one message
 * with another. It typically links a reply message with its requesting message.
 *
 * <P>
 * {@code JMSCorrelationID} can hold a provider-specific message ID, an
 * application-specific {@code String} object, or a provider-native
 * {@code byte[]} value.
 *
 ...
 *
 * <P>
 * Property values can be {@code boolean}, {@code byte}, {@code short},
 * {@code int}, {@code long}, {@code float}, {@code double}, and {@code String}.
 *
 * <P>
 ...
 * properties at this point, a {@code MessageNotWriteableException} is thrown.
 * If {@code clearProperties} is called, the properties can now be both read
 * from and written to. Note that header fields are distinct from properties.
 * Header fields are never in read-only mode.
 *
 * <P>
 ...
 * must be supported. The unmarked cases must throw a {@code JMSException}. The
 * {@code String}-to-primitive conversions may throw a runtime exception if the
 * primitive's {@code valueOf} method does not accept the {@code String} as a
 * valid representation of the primitive.
 *
 * <P>
 ...
 * the {@code setObjectProperty} and {@code getObjectProperty} methods. These
 ...
 *
 * <P>
 * The {@code setObjectProperty} method accepts values of class {@code Boolean},
 * {@code Byte}, {@code Short}, {@code Integer}, {@code Long}, {@code Float},
 * {@code Double}, and {@code String}. An attempt to use any other class must
 * throw a {@code JMSException}.
 *
 * <P>
 * The {@code getObjectProperty} method only returns values of class
 * {@code Boolean}, {@code Byte}, {@code Short}, {@code Integer}, {@code Long},
 * {@code Float}, {@code Double}, and {@code String}.
 *
 * <P>
 * The order of property values is not defined. To iterate through a message's
 * property values, use {@code getPropertyNames} to retrieve a property name
 * enumeration and then use the various property get methods to retrieve their
 * values.
 *
 * <P>
 * A message's properties are deleted by the {@code clearProperties} method.
 ...
 * value. Only the {@code getStringProperty} and {@code getObjectProperty}
 ...
 * {@code valueOf(String)} conversion method with a null value.
 *
 * <P>
 * The JMS API reserves the {@code JMSX} property name prefix for JMS defined
 * properties. The full set of these properties is defined in the Java Message
 * Service specification. The specification also defines whether support for
 * each property is mandatory or optional. New JMS defined properties may be
 * added in later versions of the JMS API. The
 * {@code String[] ConnectionMetaData.getJMSXPropertyNames} method returns the
 ...
 * treated like any other absent property. The effect of setting a message
 * selector on a property which is set by the provider on receive is undefined.
 *
 * <P>
 ...
 *
 * <P>
 * {@code JMSXGroupID} and {@code JMSXGroupSeq} are standard properties that
 * clients should use if they want to group messages. All providers must support
 * them. Unless specifically noted, the values and semantics of the JMSX
 * properties are undefined.
 *
 * <P>
 * The JMS API reserves the <CODE>JMS_<I>vendor_name</I></CODE> property name
 * prefix for provider-specific properties. Each provider defines its own value
 * for <CODE><I>vendor_name</I></CODE>. This is the mechanism a JMS provider
 * uses to make its special per-message services available to a JMS client.
 *
 * <P>
 ...
 * {@code Session} object for creating instances of messages. This allows a
 * provider to use message implementations tailored to its specific needs.
 *
 * <P>
 ...
 * means for a message not to be delivered depends on the
 * {@code MessageConsumer} being used (see {@link javax.jms.QueueReceiver
 * QueueReceiver} and {@link javax.jms.TopicSubscriber TopicSubscriber}).
 *
 * <P>
 ...
 *
 * <P>
 * A message selector is a {@code String} whose syntax is based on a subset of
 ...
 * represented by doubled single quote; for example, {@code 'literal'} and
 * {@code 'literal''s'}. Like string literals in the Java programming language,
 * these use the Unicode character encoding.
 * <LI>An exact numeric literal is a numeric value without a decimal point, such
 * as {@code 57}, {@code -957}, and {@code +62}; numbers in the range of
 * {@code long} are supported. Exact numeric literals use the integer literal
 * syntax of the Java programming language.
 * <LI>An approximate numeric literal is a numeric value in scientific notation,
 * such as {@code 7E3} and {@code -57.9E2}, or a numeric value with a decimal,
 * such as {@code 7.}, {@code -95.7}, and {@code +6.2}; numbers in the range of
 * {@code double} are supported. Approximate literals use the floating-point
 * literal syntax of the Java programming language.
 * <LI>The boolean literals {@code TRUE} and {@code FALSE}.
 * </UL>
 * <LI>Identifiers:
 * <UL>
 * <LI>An identifier is an unlimited-length sequence of letters and digits, the
 * first of which must be a letter. A letter is any character for which the
 * method {@code Character.isJavaLetter} returns true. This includes {@code '_'}
 * and {@code '$'}. A letter or digit is any character for which the method
 * {@code Character.isJavaLetterOrDigit} returns true.
 * <LI>Identifiers cannot be the names {@code NULL}, {@code TRUE}, and
 * {@code FALSE}.
 * <LI>Identifiers cannot be {@code NOT}, {@code AND}, {@code OR},
 * {@code BETWEEN}, {@code LIKE}, {@code IN}, {@code IS}, or {@code ESCAPE}.
 ...
 * <LI>The conversions that apply to the get methods for properties do not apply
 * when a property is used in a message selector expression. For example,
 ...
 * {@code JMSDeliveryMode}, {@code JMSPriority}, {@code JMSMessageID},
 * {@code JMSTimestamp}, {@code JMSCorrelationID}, and {@code JMSType}.
 * {@code JMSMessageID}, {@code JMSCorrelationID}, and {@code JMSType} values
 * may be null and if so are treated as a {@code NULL} value.
 * <LI>Any name beginning with {@code 'JMSX'} is a JMS defined property name.
 * <LI>Any name beginning with {@code 'JMS_'} is a provider-specific property
 * name.
 ...
 * {@code true} matches; a selector that evaluates to {@code false} or unknown
 * does not match.
 * <LI>Arithmetic expressions are composed of themselves, arithmetic operations,
 * identifiers (whose value is treated as a numeric literal), and numeric
 * literals.
 * <LI>Conditional expressions are composed of themselves, comparison
 * operations, and logical operations.
 * </UL>
 * <LI>Standard bracketing {@code ()} for ordering expression evaluation is
 * supported.
 * <LI>Logical operators in precedence order: {@code NOT}, {@code AND},
 * {@code OR}
 * <LI>Comparison operators: {@code =}, {@code >}, {@code >=}, {@code <},
 * {@code <=}, {@code <>} (not equal)
 * <UL>
 * <LI>Only like type values can be compared. One exception is that it is valid
 ...
 * to {@code NULL}, the value of the expression is unknown.
 * <LI>String and boolean comparison is restricted to {@code =} and {@code <>}.
 * Two strings are equal if and only if they contain the same sequence of
 * characters.
 * </UL>
 ...
 * <LI>Arithmetic operations must use numeric promotion in the Java programming
 * language.
 * </UL>
 ...
 * <LI><CODE>"age&nbsp;NOT&nbsp;BETWEEN&nbsp;15&nbsp;AND&nbsp;19"</CODE> is
 * equivalent to
 ...
 * <CODE><I>identifier</I></CODE> has a <CODE>String</CODE> or <CODE>NULL</CODE>
 * value)
 * <UL>
 * <LI><CODE>"Country&nbsp;IN&nbsp;('&nbsp;UK',&nbsp;'US',&nbsp;'France')"</CODE>
 * is true for <CODE>'UK'</CODE> and false for <CODE>'Peru'</CODE>; it is
 * equivalent to the expression
 * <CODE>"(Country&nbsp;=&nbsp;'&nbsp;UK')&nbsp;OR&nbsp;(Country&nbsp;=&nbsp;'&nbsp;US')&nbsp;OR&nbsp;(Country&nbsp;=&nbsp;'&nbsp;France')"</CODE>
 * <LI><CODE>"Country&nbsp;NOT&nbsp;IN&nbsp;('&nbsp;UK',&nbsp;'US',&nbsp;'France')"</CODE>
 * is false for <CODE>'UK'</CODE> and true for <CODE>'Peru'</CODE>; it is
 * equivalent to the expression
 * <CODE>"NOT&nbsp;((Country&nbsp;=&nbsp;'&nbsp;UK')&nbsp;OR&nbsp;(Country&nbsp;=&nbsp;'&nbsp;US')&nbsp;OR&nbsp;(Country&nbsp;=&nbsp;'&nbsp;France'))"</CODE>
 * <LI>If identifier of an <CODE>IN</CODE> or <CODE>NOT IN</CODE> operation is
 * <CODE>NULL</CODE>, the value of the operation is unknown.
 * </UL>
 ...
 * <CODE><I>pattern-value</I></CODE> is a string literal where <CODE>'_'</CODE>
 * stands for any single character; <CODE>'%'</CODE> stands for any sequence of
 * characters, including the empty sequence; and all other characters stand for
 * themselves. The optional <CODE><I>escape-character</I></CODE> is a
 * single-character string literal whose character is used to escape the special
 * meaning of the <CODE>'_'</CODE> and <CODE>'%'</CODE> in
 * <CODE><I>pattern-value</I></CODE>.)
 * <UL>
 * <LI><CODE>"phone&nbsp;LIKE&nbsp;'12%3'"</CODE> is true for <CODE>'123'</CODE>
 * or <CODE>'12993'</CODE> and false for <CODE>'1234'</CODE>
 * <LI><CODE>"word&nbsp;LIKE&nbsp;'l_se'"</CODE> is true for <CODE>'lose'</CODE>
 * and false for <CODE>'loose'</CODE>
 * <LI><CODE>"underscored&nbsp;LIKE&nbsp;'\_%'&nbsp;ESCAPE&nbsp;'\'"</CODE> is
 * true for <CODE>'_foo'</CODE> and false for <CODE>'bar'</CODE>
 * <LI><CODE>"phone&nbsp;NOT&nbsp;LIKE&nbsp;'12%3'"</CODE> is false for
 * <CODE>'123'</CODE> or <CODE>'12993'</CODE> and true for <CODE>'1234'</CODE>
 * <LI>If <CODE><I>identifier</I></CODE> of a <CODE>LIKE</CODE> or
 * <CODE>NOT LIKE</CODE> operation is <CODE>NULL</CODE>, the value of the
 ...
 * incorrect selector must result in a {@code JMSException}. JMS providers may
 * also optionally provide some semantic checking at the time the selector is
 * presented. Not all semantic checking can be performed at the time a message
 * selector is presented, because property types are not known.
 * 
 * <P>
 ...
 *
 * <P>
 * As noted above, property values may be {@code NULL}. The evaluation of
 * selector expressions containing {@code NULL} values is defined by SQL92
 * {@code NULL} semantics. A brief description of these semantics is provided
 * here.
 *
 * <P>
 * SQL treats a {@code NULL} value as unknown. Comparison or arithmetic with an
 * unknown value always yields an unknown value.
 *
 * <P>
 * The {@code IS NULL} and {@code IS NOT NULL} operators convert an unknown
 * value into the respective {@code TRUE} and {@code FALSE} values.
 *
 * <P>
 ...
 *
 * <P>
 * When used in a message selector, the {@code JMSDeliveryMode} header field is
 * treated as having the values {@code 'PERSISTENT'} and
 * {@code 'NON_PERSISTENT'}.
 *
 * <P>
 * Date and time values should use the standard {@code long} millisecond value.
 * When a date or time literal is included in a message selector, it should be
 * an integer literal for a millisecond value. The standard way to produce
...
public interface Message {
	...
	/**
	 * The message producer's default time to live is unlimited; the message never
	 * expires.
	 */
	 ...
	 *
	 * <P>
	 * The {@code JMSMessageID} header field contains a value that uniquely
	 * identifies each message sent by a provider.
	 * 
	 * <P>
	 * When a message is sent, {@code JMSMessageID} can be ignored. When the
	 * {@code send} or {@code publish} method returns, it contains a
	 * provider-assigned value.
	 *
	 * <P>
	 * A {@code JMSMessageID} is a {@code String} value that should function as a
	 ...
	 * messages for a specific installation of a provider, where an installation is
	 * some connected set of message routers.
	 *
	 * <P>
	 * All {@code JMSMessageID} values must start with the prefix {@code 'ID:'}.
	 ...
	 * some JMS providers may be able to optimize message overhead if they are given
	 * a hint that the message ID is not used by an application. By calling the
	 * {@code MessageProducer.setDisableMessageID} method, a JMS client enables this
	 * potential optimization for all messages sent by that message producer. If the
	 * JMS provider accepts this hint, these messages must have the message ID set
	 * to null; if the provider ignores the hint, the message ID must be set to its
	 * normal unique value.
	 *
	 * @return the message ID
	 *
	 * @exception JMSException if the JMS provider fails to get the message ID due
	 *                         to some internal error.
	 * @see javax.TextMessageImpl.jms.Message#setJMSMessageID(String)
	 ...
	 * is sent. This message cannot be used by clients to configure the message ID.
	 * This method is public to allow a JMS provider to set this field when sending
	 * a message whose implementation is not its own.
	 *
	 * @param id the ID of the message
	 *
	 * @exception JMSException if the JMS provider fails to set the message ID due
	 *                         to some internal error.
	 *
	 * @see javax.TextMessageImpl.jms.Message#getJMSMessageID()
	 */
	 ...
	 * 
	 * <P>
	 * The {@code JMSTimestamp} header field contains the time a message was handed
	 * off to a provider to be sent. It is not the time the message was actually
	 * transmitted, because the actual send may occur later due to transactions or
	 * other client-side queueing of messages.
	 *
	 * <P>
	 * When a message is sent, {@code JMSTimestamp} is ignored. When the
	 * {@code send} or {@code publish} method returns, it contains a time value
	 * somewhere in the interval between the call and the return. The value is in
	 ...
	 * some JMS providers may be able to optimize message overhead if they are given
	 * a hint that the timestamp is not used by an application. By calling the
	 * {@code MessageProducer.setDisableMessageTimestamp} method, a JMS client
	 * enables this potential optimization for all messages sent by that message
	 * producer. If the JMS provider accepts this hint, these messages must have the
	 * timestamp set to zero; if the provider ignores the hint, the timestamp must
	 * be set to its normal value.
	 *
	 * @return the message timestamp
	 *
	 * @exception JMSException if the JMS provider fails to get the timestamp due to
	 *                         some internal error.
	 *
	 * @see javax.TextMessageImpl.jms.Message#setJMSTimestamp(long)
	 ...
	 * <P>
	 * This method is for use by JMS providers only to set this field when a message
	 * is sent. This message cannot be used by clients to configure the message
	 * timestamp. This method is public to allow a JMS provider to set this field
	 * when sending a message whose implementation is not its own.
	 *
	 * @param timestamp the timestamp for this message
	 * 
	 * @exception JMSException if the JMS provider fails to set the timestamp due to
	 *                         some internal error.
	 *
	 * @see javax.TextMessageImpl.jms.Message#getJMSTimestamp()
	 */
	 ...
	 *
	 * @exception JMSException if the JMS provider fails to get the correlation ID
	 *                         due to some internal error.
	 * 
	 * @see javax.TextMessageImpl.jms.Message#setJMSCorrelationID(String)
	 * @see javax.TextMessageImpl.jms.Message#getJMSCorrelationID()
	 * @see javax.TextMessageImpl.jms.Message#setJMSCorrelationIDAsBytes(byte[])
	 */
	 ...
	 * 
	 * <P>
	 * The array is copied before the method returns, so future modifications to the
	 * array will not alter this message header.
	 * 
	 * <P>
	 * If a provider supports the native concept of correlation ID, a JMS client may
	 * need to assign specific {@code JMSCorrelationID} values to match those
	 * expected by native messaging clients. JMS providers without native
	 ...
	 * 
	 * @exception JMSException if the JMS provider fails to set the correlation ID
	 *                         due to some internal error.
	 * 
	 * @see javax.TextMessageImpl.jms.Message#setJMSCorrelationID(String)
	 * @see javax.TextMessageImpl.jms.Message#getJMSCorrelationID()
	 * @see javax.TextMessageImpl.jms.Message#getJMSCorrelationIDAsBytes()
	 */
	 ...
	 * 
	 * <P>
	 * Since each message sent by a JMS provider is assigned a message ID value, it
	 * is convenient to link messages via message ID. All message ID values must
	 * start with the {@code 'ID:'} prefix.
	 * 
	 * <P>
	 ...
	 * may use {@code JMSCorrelationID} to hold a value referencing some external
	 * information. Application-specified values must not start with the
	 * {@code 'ID:'} prefix; this is reserved for provider-generated message ID
	 * values.
	 * 
	 * <P>
	 * If a provider supports the native concept of correlation ID, a JMS client may
	 ...
	 * used for this purpose. JMS providers without native correlation ID values are
	 * not required to support {@code byte[]} values. The use of a {@code byte[]}
	 * value for {@code JMSCorrelationID} is non-portable.
	 * 
	 ...
	 * provider-specific message IDs or application-specific {@code String} values.
	 *
	...
	/**
	 * Gets the {@code Destination} object to which a reply to this message should
	 * be sent.
	 * 
	 ...
	 *                         {@code JMSReplyTo} destination due to some internal
	 *                         error.
	 *
	 * @see javax.TextMessageImpl.jms.Message#setJMSReplyTo(Destination)
	 */

	Destination getJMSReplyTo() throws JMSException;

	/**
	 * Sets the {@code Destination} object to which a reply to this message should
	 * be sent.
	 * 
	 * <P>
	 * The {@code JMSReplyTo} header field contains the destination where a reply to
	 * the current message should be sent. If it is null, no reply is expected. The
	 * destination may be either a {@code Queue} object or a {@code Topic} object.
	 *
	 * <P>
	 ...
	 *
	 * @param replyTo {@code Destination} to which to send a response to this
	 *                message
	 *
	 ...
	 *                         error.
	 *
	 * @see javax.TextMessageImpl.jms.Message#getJMSReplyTo()
	 */
	 ...
	 * 
	 * <P>
	 * The {@code JMSDestination} header field contains the destination to which the
	 * message is being sent.
	 * 
	 * <P>
	 * When a message is sent, this field is ignored. After completion of the
	 * {@code send} or {@code publish} method, the field holds the destination
	 * specified by the method.
	 * 
	 * <P>
	 * When a message is received, its {@code JMSDestination} value must be
	 * equivalent to the value assigned when it was sent.
	 *
	 * @return the destination of this message
	 * 
	 * @exception JMSException if the JMS provider fails to get the destination due
	 *                         to some internal error.
	 * 
	 * @see javax.TextMessageImpl.jms.Message#setJMSDestination(Destination)
	 */
	 ...
	 * <P>
	 * This method is for use by JMS providers only to set this field when a message
	 * is sent. This message cannot be used by clients to configure the destination
	 * of the message. This method is public to allow a JMS provider to set this
	 * field when sending a message whose implementation is not its own.
	 *
	 * @param destination the destination for this message
	 * 
	 * @exception JMSException if the JMS provider fails to set the destination due
	 *                         to some internal error.
	 * 
	 * @see javax.TextMessageImpl.jms.Message#getJMSDestination()
	 */
	 ...
	 * 
	 * @exception JMSException if the JMS provider fails to get the delivery mode
	 *                         due to some internal error.
	 * 
	 * @see javax.TextMessageImpl.jms.Message#setJMSDeliveryMode(int)
	 * @see javax.javax.jms.DeliveryMode
	 */
	 ...
	 * <P>
	 * This method is for use by JMS providers only to set this field when a message
	 * is sent. This message cannot be used by clients to configure the delivery
	 * mode of the message. This method is public to allow a JMS provider to set
	 * this field when sending a message whose implementation is not its own.
	 *
	 * @param deliveryMode the delivery mode for this message
	 * 
	 * @exception JMSException if the JMS provider fails to set the delivery mode
	 *                         due to some internal error.
	 * 
	 * @see javax.TextMessageImpl.jms.Message#getJMSDeliveryMode()
	 * @see javax.javax.jms.DeliveryMode
	 */
	 ...
	 *
	 * <P>
	 * If a client receives a message with the {@code JMSRedelivered} field set, it
	 * is likely, but not guaranteed, that this message was delivered earlier but
	 ...
	 *
	 * @see javax.TextMessageImpl.jms.Message#setJMSRedelivered(boolean)
	 */
	 ...
	 * <P>
	 * This method is for use by JMS providers only to set this field when a message
	 * is delivered. This message cannot be used by clients to configure the
	 * redelivered status of the message. This method is public to allow a JMS
	 * provider to set this field when sending a message whose implementation is not
	 * its own.
	 *
	 ...
	 *                         state due to some internal error.
	 *
	 * @see javax.TextMessageImpl.jms.Message#getJMSRedelivered()
	 */
	 ...
	 * 
	 * @exception JMSException if the JMS provider fails to get the message type due
	 *                         to some internal error.
	 *
	 * @see javax.TextMessageImpl.jms.Message#setJMSType(String)
	 */
	 ...
	 * messages sent by applications. The {@code JMSType} header field may reference
	 * a message's definition in the provider's repository.
	 *
	 * <P>
	 ...
	 * {@code JMSType}, whether the application makes use of it or not. This ensures
	 * that the field is properly set for those providers that require it.
	 *
	 * <P>
	 ...
	 * defined in the current provider's message repository. If string literals are
	 * used, they may not be valid type names for some JMS providers.
	 *
	 * @param type the message type
	 * 
	 * @exception JMSException if the JMS provider fails to set the message type due
	 *                         to some internal error.
	 *
	 * @see javax.TextMessageImpl.jms.Message#getJMSType()
	 */
	 ...
	 * 
	 * <P>
	 * When a message is sent, the {@code JMSExpiration} header field is left
	 * unassigned. After completion of the {@code send} or {@code publish} method,
	 * it holds the expiration time of the message. This is the the difference,
	 * measured in milliseconds, between the expiration time and midnight, January
	 * 1, 1970 UTC.
	 *
	 * <P>
	 * If the time-to-live is specified as zero, {@code JMSExpiration} is set to
	 ...
	 *
	 * @see javax.TextMessageImpl.jms.Message#setJMSExpiration(long)
	 */
	 ...
	 * <P>
	 * This method is for use by JMS providers only to set this field when a message
	 * is sent. This message cannot be used by clients to configure the expiration
	 * time of the message. This method is public to allow a JMS provider to set
	 * this field when sending a message whose implementation is not its own.
	 * 
	 ...
	 *                         expiration due to some internal error.
	 *
	 * @see javax.TextMessageImpl.jms.Message#getJMSExpiration()
	 */
	 ...
	 * 
	 * <P>
	 * When a message is sent, the {@code JMSDeliveryTime} header field is left
	 * unassigned. After completion of the {@code send} or {@code publish} method,
	 * it holds the delivery time of the message. This is the the difference,
	 * measured in milliseconds, between the delivery time and midnight, January 1,
	 * 1970 UTC.
	 * <p>
	 ...
	 * 
	 * @exception JMSException if the JMS provider fails to get the delivery time
	 *                         due to some internal error.
	 * 
	 * @see javax.TextMessageImpl.jms.Message#setJMSDeliveryTime(long)
	 * 
	 * @since JMS 2.0
	 */
	 ...
	 * 
	 * @exception JMSException if the JMS provider fails to set the delivery time
	 *                         due to some internal error.
	 * 
	 * @see javax.TextMessageImpl.jms.Message#getJMSDeliveryTime()
	 * 
	 * @since JMS 2.0
	 */
	 ...
	 * 
	 * @exception JMSException if the JMS provider fails to get the message priority
	 *                         due to some internal error.
	 *
	 * @see javax.TextMessageImpl.jms.Message#setJMSPriority(int)
	 */
	 ...
	 * <P>
	 * This method is for use by JMS providers only to set this field when a message
	 * is sent. This message cannot be used by clients to configure the priority
	 * level of the message. This method is public to allow a JMS provider to set
	 * this field when sending a message whose implementation is not its own.
	 *
	 * @param priority the priority of this message
	 * 
	 * @exception JMSException if the JMS provider fails to set the message priority
	 *                         due to some internal error.
	 *
	 * @see javax.TextMessageImpl.jms.Message#getJMSPriority()
	 */
	 ...
	 * 
	 * @exception JMSException           if the JMS provider fails to get the
	 *                                   property value due to some internal error.
	 ...
	 * 
	 * @return the {@code String} property value for the specified name; if there is
	 *         no property by this name, a null value is returned
	 * 
	 * @exception JMSException           if the JMS provider fails to get the
	 *                                   property value due to some internal error.
	 * @exception MessageFormatException if this type conversion is invalid.
	 ...
	 *         {@code int}, an {@code Integer} is returned); if there is no property
	 *         by this name, a null value is returned
	 * 
	 * @exception JMSException if the JMS provider fails to get the property value
	 *                         due to some internal error.
	 */
	 ...
	 * 
	 * @exception JMSException if the JMS provider fails to get the property names
	 *                         due to some internal error.
	 */
	 ...
	 * 
	 * @exception JMSException                 if the JMS provider fails to set the
	 *                                         property due to some internal error.
	 ...
	 * 
	 * <P>
	 * All consumed JMS messages support the {@code acknowledge} method for use when
	 * a client has specified that its JMS session's consumed messages are to be
	 * explicitly acknowledged. By invoking {@code acknowledge} on a consumed
	 * message, a client acknowledges all messages consumed by the session that the
	 * message was delivered to.
	 * 
	 * <P>
	 * Calls to {@code acknowledge} are ignored for both transacted sessions and
	 ...
	 * may choose to acknowledge messages as an application-defined group (which is
	 * done by calling acknowledge on the last received message of the group,
	 ...
	 *
	 * @exception JMSException if the JMS provider fails to clear the message body
	 *                         due to some internal error.
	 */
	...
	/**
	 * Returns the message body as an object of the specified type. This method may
	 * be called on any type of message except for <tt>StreamMessage</tt>. The
	 * message body must be capable of being assigned to the specified type. This
	 * means that the specified class or interface must be either the same as, or a
	 ...
	 *          If the message is a {@code TextMessage} then this parameter must be
	 *          set to {@code String.class} or another type to which a
	 *          {@code String} is assignable. <br>
	 *          If the message is a {@code ObjectMessage} then parameter must must
	 *          be set to {@code java.io.Serializable.class} or another type to
	 *          which the body is assignable. <br>
	 *          If the message is a {@code MapMessage} then this parameter must be
	 *          set to {@code java.util.Map.class} (or
	 *          {@code java.lang.Object.class}). <br>
	 *          If the message is a {@code BytesMessage} then this parameter must be
	 *          set to {@code byte[].class} (or {@code java.lang.Object.class}).
	 *          This method will reset the {@code BytesMessage} before and after
	 *          use.<br>
	 *          If the message is a {@code TextMessage}, {@code ObjectMessage},
	 *          {@code MapMessage} or {@code BytesMessage} and the message has no
	 *          body, then the above does not apply and this parameter may be set to
	 *          any type; the returned value will always be null.<br>
	 *          If the message is a {@code Message} (but not one of its subtypes)
	 *          then this parameter may be set to any type; the returned value will
	 *          always be null.
	 * 
	 ...
	 *                                   <ul>
	 *                                   <li>if the message is a
	 *                                   {@code StreamMessage}
	 *                                   <li>if the message body cannot be assigned
	 *                                   to the specified type
	 *                                   <li>if the message is an
	 *                                   {@code ObjectMessage} and object
	 *                                   deserialization fails.
	 *                                   </ul>
	 * 
	 * @exception JMSException           if the JMS provider fails to get the
	 *                                   message body due to some internal error.
	 * 
	 * @since JMS 2.0
	 */
	 ...
	 * method {@code getBody} on the same message with the same type argument would
	 * not throw a MessageFormatException.
	 * <p>
	 * If the message is a {@code StreamMessage} then false is always returned. If
	 * the message is a {@code ObjectMessage} and object deserialization fails then
	 * false is returned. If the message has no body then any type may be specified
	 * and true is returned.
	 * 
	 * @param c The specified type <br>
	 *          If the message is a {@code TextMessage} then this method will only
	 *          return true if this parameter is set to {@code String.class} or
	 *          another type to which a {@code String} is assignable. <br>
	 *          If the message is a {@code ObjectMessage} then this method will only
	 *          return true if this parameter is set to
	 *          {@code java.io.Serializable.class} or another class to which the
	 *          body is assignable. <br>
	 *          If the message is a {@code MapMessage} then this method will only
	 *          return true if this parameter is set to {@code java.util.Map.class}
	 *          (or {@code java.lang.Object.class}). <br>
	 *          If the message is a {@code BytesMessage} then this this method will
	 *          only return true if this parameter is set to {@code byte[].class}
	 *          (or {@code java.lang.Object.class}). <br>
	 *          If the message is a {@code TextMessage}, {@code ObjectMessage},
	 *          {@code MapMessage} or {@code BytesMessage} and the message has no
	 *          body, then the above does not apply and this method will return true
	 *          irrespective of the value of this parameter.<br>
	 *          If the message is a {@code Message} (but not one of its subtypes)
	 *          then this method will return true irrespective of the value of this
	 *          parameter.
	 * 
	 * @return whether the message body is capable of being assigned to the
	 *         specified type
	 * 
	 * @exception JMSException if the JMS provider fails to return a value due to
	 *                         some internal error.
	 */
	boolean isBodyAssignableTo(Class<? extends Message> c) throws JMSException;
}